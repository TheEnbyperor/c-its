...
fn handle_802_11_frame(data: &[u8]) {
    ...
    let frame_data = match &geo_networking_frame.data {
        geo_networking::PacketType::Unsecured(ch) => std::borrow::Cow::Borrowed(ch),
    ...
    let frame_out = data_structures::ITSFrame {
        ...
        geo_networking: data_structures::GeoNetworkingFrame {
            ...
            header: match &frame_data.data {
                geo_networking::PacketData::GeoAnycast(p) => data_structures::PacketHeader::GeoAnycast {
                    source_position: (&p.source_position).into(),
                    sequence: p.sequence,
                    area_centre: p.area_centre,
                    area: p.area
                },
                geo_networking::PacketData::GeoBroadcast(p) => data_structures::PacketHeader::GeoBroadcast {
                    source_position: (&p.source_position).into(),
                    sequence: p.sequence,
                    area_centre: p.area_centre,
                    area: p.area
            ...
            data: match frame_inner_data {
                Some(d) => Some(match d {
                    geo_networking::InnerData::BtpA { source_port, destination_port, data} => data_structures::PacketData::BtpA {
                        source_port: *source_port,
                        destination_port: *destination_port,
                        data: data.as_ref()
                    },
                    geo_networking::InnerData::BtpB { destination_port, destination_port_info, data} => data_structures::PacketData::BtpB {
                        destination_port: *destination_port,
                        destination_port_info: *destination_port_info,
                        data: build_inner_data(*destination_port, data.as_ref())
                    },
                    geo_networking::InnerData::Ipv6(data) => data_structures::PacketData::Ipv6(data.as_ref())
                }),
                None => None,
            }
        }
...
fn build_inner_data(port: u16, data: &[u8]) -> data_structures::InnerData {
    if port == 2001 {
        match rasn::uper::decode::<c_its::asn::cam_pdu_descriptions::CAM>(data) {
            Ok(cam) => data_structures::InnerData::CAM(serde_json::from_str(&rasn::jer::encode(&cam).unwrap()).unwrap()),
            Err(_) => data_structures::InnerData::Raw(data)
        }
    } else if port == 2002 {
        match rasn::uper::decode::<c_its::asn::denm_pdu_description::DENM>(data) {
            Ok(cam) => data_structures::InnerData::DENM(serde_json::from_str(&rasn::jer::encode(&cam).unwrap()).unwrap()),
            Err(_) => data_structures::InnerData::Raw(data)
        }
    } else if port == 2003 {
        match rasn::uper::decode::<c_its::asn::mapem_pdu_descriptions::MAPEM>(data) {
            Ok(cam) => data_structures::InnerData::MAPEM(serde_json::from_str(&rasn::jer::encode(&cam).unwrap()).unwrap()),
            Err(_) => data_structures::InnerData::Raw(data)
        }
    } else if port == 2004 {
        match rasn::uper::decode::<c_its::asn::spatem_pdu_descriptions::SPATEM>(data) {
            Ok(cam) => data_structures::InnerData::SPATEM(serde_json::from_str(&rasn::jer::encode(&cam).unwrap()).unwrap()),
            Err(_) => data_structures::InnerData::Raw(data)
        }
    } else if port == 2006 {
        match rasn::uper::decode::<c_its::asn::ivim_pdu_descriptions::IVIM>(data) {
            Ok(cam) => data_structures::InnerData::IVIM(serde_json::from_str(&rasn::jer::encode(&cam).unwrap()).unwrap()),
            Err(_) => data_structures::InnerData::Raw(data)
        }
    } else {
